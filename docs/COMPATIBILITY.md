...
| **Turbo** (CCSDS 標準) | 完全実装：系統ビット + RSC エンコード (K=5, g0=23₈, g1=33₈) + CCSDS Quadratic‑Permutation インタリーバ、パンクチュアパターン、Iterative Log‑MAP デコーダ（最大10回） | 完全実装（標準インタリーバ、パンクチュア、MAP デコーダ） | 完全実装（同上） | 完全実装（同上） | 完全実装（同上） | 完全実装（同上） | 未実装 | **準拠実装** – CCSDS 131.0‑B‑4 準拠（QPP インタリーバ・Log‑MAP デコーダ）。外部実装（gr-ccsds-1/SatDump）とのバイト単位一致はゴールデンベクトル検証待ち（リスク項目 G-4）。 |
