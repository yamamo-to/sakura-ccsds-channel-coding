...
| TURBO-05 | §6.3g | QPP インタリーバ（Quadratic-Permutation） | `core/interleaver.py` (`ccsds_perm` / `ccsds_interleaver` / `ccsds_deinterleaver`) | `test_turbo_extended.py::test_interleaver_is_bijective_and_inverse`, `test_turbo_properties.py::test_interleaver_is_permutation`, `test_turbo_properties.py::test_deinterleaver_is_inverse`, `test_turbo_golden.py::test_ccsds_perm_1784_matches_reference` | ✅ 準拠 | 全単射・逆元 + **K=1784 は外部参照ゴールデンベクトル照合済み**（`tests/data/ccsdsSize1784.txt`、mdmoctezuma/CCSDSTurboCode 由来、1784 点完全一致）。k1=8 固定・k2=K/8 は 131.0-B-4 Table 6-3 と一致（全 4 標準長） |
| TURBO-06 | §3.4 (Annex) | 反復 Log-MAP / Max-Log-MAP 復号 | `core/turbo.py:215` (`_build_trellis`), `:256` (`_bcjr_kernel`), `_turbo_decode_core` | `test_turbo.py::test_unpunctured_roundtrip`, `test_turbo.py::test_punctured_roundtrip`, `test_turbo_extended.py::test_decode_consistency_across_iterations` | ✅ 準拠 |  **全 4 標準長 × rate 1/3/1/4/1/6 は外部参照デコーダとビット一致**（`test_turbo_decode_matches_reference_decoder`、deepspace-turbo、誤り 8/30 ビット注入。rate 1/2 は参照実装の非標準パンクチャのため除外） |
...
| G-3 | Turbo 標準ブロック長での実符復号テスト不在 | 高 | **解消** – `test_turbo_standard_lengths.py` で全 5 長 × rate 1/3・1/6 の roundtrip + 全 5 長 × 4 レートの自動判別を検証。※CCSDS 131.0-B-4 Table 6-1 の標準長は 4 つ（1784/3568/7136/8920）。16384 は規格上 Turbo 標準長に存在しないため拡張扱い |
| G-4 | Turbo QPP インタリーバの外部照合 | 中 | **解消** – 公式 CCSDS §6.3g 数式の独立再実装と全 4 標準長（1784/3568/7136/8920）を照合。K=1784 はさらに外部参照ファイルと一致 |
| G-4b | Turbo Annex A 符号語ベクトル不在 | 中 | **解消** – `geeanlooca/deepspace-turbo` 参照実装の rate-1/3 出力に CCSDS §3.4 puncturing を適用し、rate 1/2 を含む全 rates・全 4 標準長のエンコーダ出力を照合 |
| G-4c | Turbo デコーダの外部参照照合 | 中 | **解消** – 全 4 標準長（1784/3568/7136/8920）× rate 1/3/1/4/1/6（誤り 8/30 ビット注入）で `geeanlooca/deepspace-turbo` 参照デコーダとビット一致（`test_turbo_decode_matches_reference_decoder`）。rate 1/2 は参照実装が非標準パンクチャのため除外。※K=16384 は CCSDS 標準ブロック長に**存在しない**（131.0-B-4 Table 6-1 は 4 長のみ、16384 は LDPC §7.4 の長）ため照合対象外 |
